...
namespace sg14 {
    namespace _impl {
       ...
       // higher OP number_base<>
        ...
        auto operate(const Lhs& lhs, const number_base<RhsDerived, RhsRep>& rhs, Operator op) {
		//-> decltype(op(_impl::from_value<RhsDerived>(lhs), static_cast<const RhsDerived&>(rhs))) {
			auto lft = from_value<RhsDerived>(lhs);
			auto rht = static_cast<const RhsDerived&>(rhs);
